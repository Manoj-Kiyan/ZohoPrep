 ...
 * Each node can be either a file or a directory and contains relevant information.
 */
class FileNode {
    ...
    // Getters and setters
    public String getName() { return name; }                                     // Get file/directory name
    public void setName(String name) { this.name = name; }                      // Update file/directory name
    public boolean isDirectory() { return isDirectory; }                         // Check if it's a directory
    public String getContent() { return content; }                              // Get file content
    public void setContent(String content) { this.content = content; }          // Update file content
    public Map<String, FileNode> getChildren() { return children; }             // Get child nodes
    public FileNode getParent() { return parent; }                             // Get parent node
    public boolean isDeleted() { return isDeleted; }                           // Check if node is deleted
    public void setDeleted(boolean deleted) { this.isDeleted = deleted; }      // Mark node as deleted/restored
}

/**
 * FileManagementSystem implements an in-memory file system with basic file operations
 * Supports operations like create, delete, rename, update, and restore files/directories
 */
public class FileManagementSystem {
    ...
    private void printTree(FileNode node, int level) {                     // Helper to print tree structure
        if (node.isDeleted()) return;                                      // Skip deleted nodes
    ...
    public void viewContent(String path) {
    FileNode node = findNode(path);
    if (node == null || node.isDirectory()) {
        System.out.println("File not found!");
        return;
    }
    System.out.println("Content of " + node.getName() + ": " + node.getContent());
}
    ...
    private FileNode findNode(String path) {                        // Find node by path (e.g., "documents/report.txt")
        if (path.equals("root")) return root;                      // Return root if requested
    ...
    public static void main(String[] args) {                     // Main program entry point
        ...
        while (true) {                                           // Main program loop
            ...
            System.out.println("7. View Content");               // Menu option 7
            System.out.println("8. Exit");                       // Menu option 8
